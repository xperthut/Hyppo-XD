...
namespace hyppox {
    class Hyppox:public HInterface{
        ...
        std::string getD3GraphObject(const std::vector<std::string> argv);
    ...
    std::string Hyppox::getD3GraphObject(int argc, const char * argv[]){
        ...
        try{
            while(index<argc){
                argV.push_back(argv[index]);
                index++;
            }
        }catch (std::runtime_error &e) {
            std::cout<<"\nError to read parameters (Runtime error): "<<e.what()<<std::endl;
            return "Format missing:" + std::string(e.what());
        ...
        short ret = this->parseArgs(argV, genotypeCol, filterGen, locationCol, dateTimeCol, otherInfoCol, memCol, pieCol, filterCol, clusterCol, clusterIndexForEdgeDirection, clusterName, clusterParams, windows, overlap, readDir, writeDir, minPathLen, deltaChanges, increaseTime, signatureMatch, printBarcode, printBarcodeUsingJavaplex, refPerformance, filterRefIndex, intPathWidth, edgeColor, nodeSizeRange, heatmapEachConComp, fileName, sortFilterIndex, filterSignatureMatching);
        ...
        return this->getD3GraphObject(genotypeCol, filterGen, locationCol, dateTimeCol, otherInfoCol, memCol, pieCol, filterCol, clusterCol, clusterIndexForEdgeDirection, clusterName, clusterParams, windows, overlap, readDir, writeDir, minPathLen, deltaChanges, increaseTime, signatureMatch, printBarcode, printBarcodeUsingJavaplex, refPerformance, filterRefIndex, intPathWidth, edgeColor, nodeSizeRange, heatmapEachConComp, fileName, sortFilterIndex, filterSignatureMatching);
    }

    std::string Hyppox::getD3GraphObject(const std::vector<std::string> argv){
      short genotypeCol ;
      std::vector<std::string> filterGen;
      std::vector<short> locationCol;
      std::vector<short> dateTimeCol;
    ...
    std::string Hyppox::getD3GraphObject(short genotypeCol, std::vector<std::string> filterGen, std::vector<short> locationCol, std::vector<short> dateTimeCol, std::vector<short> otherInfoCol, std::vector<short> memCol, std::vector<short> pieCol, std::vector<short> filterCol, std::vector<short> clusterCol, short clusterIndexForEdgeDirection, std::string clusterName, std::vector<float> clusterParams, std::vector<short> windows, std::vector<float> overlap, std::string readDir, std::string writeDir, short minPathLen, float deltaChanges, bool increaseTime, bool signatureMatch, bool printBarcode, bool printBarcodeUsingJavaplex, bool refPerformance, short filterRefIndex, short intPathWidth, std::string edgeColor, std::vector<float> nodeSizeRange, bool heatmapEachConComp, std::string fileName, short sortFilterIndex, std::vector<short> filterSignatureMatching){
        ...
        hyppox::Config::setParams(genotypeCol, filterGen, locationCol, dateTimeCol, otherInfoCol, memCol, pieCol, filterCol, clusterCol, clusterIndexForEdgeDirection, clusterName, clusterParams, windows, overlap, readDir, writeDir, minPathLen, deltaChanges, increaseTime, signatureMatch, printBarcode, printBarcodeUsingJavaplex, refPerformance, filterRefIndex, intPathWidth, edgeColor, nodeSizeRange, heatmapEachConComp, fileName, sortFilterIndex,filterSignatureMatching);
    ...
    /*
     ...
     -MEMC: Membership indicator columns
     -PIEC: Pie chart indicator columns
     -FC: Filter columns
    ...
    void Hyppox::printHelpMessage(){
        ...
        <<"-MEMC: Membership indicator columns"<<std::endl
        <<"-PIEC: Pie chart indicator columns"<<std::endl
    ...
    int Hyppox::parseArgs(const std::vector<std::string> argv,
                           short &genotypeCol,
                           std::vector<std::string> &filterGen,
                           std::vector<short> &locationCol,
                           std::vector<short> &dateTimeCol,
        ...
        try{
            while(index<argv.size()){
                ...
                //cout<<s<<" "<<argv[index+1]<<" ";
                if(s.compare("-help")==0 || s.compare("-h")==0){
                    printHelpMessage();
                    return 2;
                ...
                }else if(s.compare("-MEMC")==0){
                    index++;
                    s = argv[index];
                    parseIndex(s, memCol);

                }else if(s.compare("-PIEC")==0){
                    index++;
                    s = argv[index];
                    parseIndex(s, pieCol);
        ...
        while(overlap.size()<(size_t)_fSize){
            overlap.push_back(overlap[0]);
        }

        if((size_t)clusterIndexForEdgeDirection >= clusterCol.size()){
    ...
    std::string Hyppox::createMapper(){
        ...
        std::string r = mapper->createMapper(&this->graph);
        delete mapper;
    ...
    std::string Hyppox::visualization(){
        ...
        std::cout<<"Total time to construct all paths:"<<_time[0]<<_time[1]<<std::endl;

        if(hyppox::Config::COL_MEMBERSHIP.size()>0){
            std::cout<<"Started constructing flares...\n";
            t1=clock();
            this->graph.generateFlares();
            t2 = clock();
